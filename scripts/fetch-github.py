...
def resolve_github_token() -> Optional[str]:
    ...
    logging.info(
        f"🔍 GH_APP_KEY_FILE: {'set' if key_file else 'not set'}{' (file exists)' if key_file and os.path.exists(key_file) else ' (file missing)' if key_file else ''}"
    )
    ...
    try:
        ...
        if token and result.returncode == 0:
            ...
            logging.info(
                f"🔍 gh auth token: exit={result.returncode}, output={'set' if token else 'empty'}"
            )
    except Exception as e:
    ...
    # 4. Unauthenticated
    logging.warning(
        "⚠️ No GitHub token found — rate limit 60 req/hr (22 repos may fail)"
    )
...
def load_sources(
    defaults_dir: Path, config_dir: Optional[Path] = None
) -> List[Dict[str, Any]]:
    ...
    for source in all_sources:
        if source.get("type") == "github" and source.get("enabled", True):
            ...
            if not source.get("repo"):
                logging.warning(
                    f"GitHub source {source.get('id', 'unknown')} missing repo field, skipping"
                )
                continue
...
Environment Variables:
    ...
    if not args.output:
        fd, temp_path = tempfile.mkstemp(
            prefix="tech-news-digest-github-", suffix=".json"
        )
        os.close(fd)
    ...
    try:
        ...
        if (
            args.config
            and args.defaults == Path("config/defaults")
            and not args.defaults.exists()
        ):
        ...
        logger.info(
            f"Fetching {len(sources)} GitHub repositories (window: {args.hours}h)"
        )
        ...
        logger.info(
            f"✅ Done: {ok_count}/{len(results)} repos ok, "
...
TRENDING_QUERIES = [
    ...
    {
        "topic": "frontier-tech",
        "q": "machine-learning deep-learning in:topics,name,description",
    },
]
...
def fetch_trending_repos(
    ...
    for tq in TRENDING_QUERIES:
        ...
        try:
            ...
            for item in data.get("items", []):
                ...
                age_days = (
                    max(1, (datetime.now(timezone.utc) - created).days)
                    if created
                    else 365
                )
            ...
            logging.debug(
                f"Trending [{tq['topic']}]: {len(data.get('items', []))} repos"
            )
            time.sleep(0.5)  # Rate limit courtesy

        except HTTPError as e:
            logging.warning(
                f"GitHub trending search error [{tq['topic']}]: HTTP {e.code}"
            )
        except Exception as e:
    ...
    logging.info(
        f"🔥 Trending: {len(all_repos)} repos found across {len(TRENDING_QUERIES)} topics"
    )
    return all_repos
...
def cmd_trending():
    ...
    parser.add_argument(
        "--hours", type=int, default=48, help="Lookback window (default: 48)"
    )
    parser.add_argument(
        "--min-stars", type=int, default=50, help="Minimum stars (default: 50)"
    )
    parser.add_argument(
        "--per-topic", type=int, default=15, help="Max repos per topic (default: 15)"
    )
    ...
    out_path = args.output or Path(
        tempfile.mkstemp(prefix="td-trending-", suffix=".json")[1]
    )
